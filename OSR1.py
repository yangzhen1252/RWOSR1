...
from loss_reweighting1 import predict_class
import models
from ops.config import parser
from training.schedule import lr_setter
from training.train import train
from training.validate1 import validate1
from training.distribute import validate1,evaluation
...
def main_worker(ngpus_per_node, args):
    ...
    valdir = os.path.join(args.data, 'unknow1')
    valdir1 = os.path.join(args.data, 'unknow2')
    ...
    val_loader1 = torch.utils.data.DataLoader(
        datasets.ImageFolder(valdir1, transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ...
    checkpoint = torch.load('resultsFU/PACS/epoch_196_checkpoint.pth.tar')
    model.load_state_dict(checkpoint['state_dict'])
    if args.evaluate:
        #validate( test_loader, model, criterion, 0, True, args, tensor_writer)
       # acc1, auc2, f1 = evaluation(model, test_loader, val_loader)
        return

    #acc=validate(test_loader, model, criterion, 0, True, args, tensor_writer)
    start_time = time.time()
    acc1, auc2, f1=evaluation(model, test_loader, val_loader,args)
    #validate1(test_loader,model, criterion,  args=args,)
