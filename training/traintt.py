...
def train(train_loader, model, criterion, optimizer, epoch, args, tensor_writer=None):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        for j in range(images.size(0)):
            ...
            predicted_label = torch.argmax(output[j]).item()

            if predicted_label == category1:
    ...
    torch.save(feature_dict, "resultstt/PACS/feature_dict_%d.pt" % (epoch))
    torch.save(feature_dict1, "resultstt/PACS/feature_dict1_%d.pt" % (epoch))
    torch.save(feature_dict2, "resultstt/PACS/feature_dict2_%d.pt" % (epoch))
